<form action="#" method="POST" enctype="multipart/form-data">
    <h2>Update News</h2>
    <input type="hidden" model:wire="selected_id">
    <div class="shadow sm:rounded-md sm:overflow-hidden">
            ...
            <div class="col-span-6 sm:col-span-3">
                ...
                <select wire:model="companies" id="companies" name="companies" autocomplete="companies" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" multiple>
                    ...
                    @foreach($allCompanies as $company)
                        <option {{ in_array($company->id, $companies) ? 'selected=selected' : '' }} value="{{$company->id}}" disabled>{{ $company->name }}</option>
                    @endforeach
                </select>
        ...
        <div class="px-4 py-3 bg-gray-50 text-right sm:px-6">
            <button wire:click.prevent="update" type="button" class="inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                Save
            </button>
        </div>
    </div>
